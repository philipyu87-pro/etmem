...
static struct page_refs *update_page_refs_in_vma_new(struct page_refs *page_refs, struct vma_info **vma_info)
{
    ...
    if (vma_info == NULL || *vma_info == NULL) {
        return NULL;
    }
    if ((*vma_info)->pages == NULL) {
        return NULL;
    }
    ...
    pf = page_refs;
    while (pf->addr >= (*vma_info)->start_addr && \
...
static void clear_old_vmas(const struct task_pid *tpid, struct vmas *vmas)
{
    ...
    struct vma_info *pre_node = NULL;
    pthread_mutex_lock(&(params->pmu_params->vma_list_mutex));
    while (node->start_addr != tmp_vma->start) {
        node = node->next;
    }
    pre_node = node;
    node = node->next;
...
static int update_page_refs_from_vma(const struct task_pid *tpid, struct page_refs **page_refs, bool flag)
{
...
static int get_sample_event(struct perf_cpu_monitor *cpu_monitor, uint64_t *address)
{
    ...
    while (tail < head) {
        ...
        if (cpu_monitor->size <= (int)sizeof(struct perf_sample)) {
            continue;
        }
...
static void *pmu_sample_thread(void *arg)
{
    ...
    params->pmu_params->g_threads_meta_set[cpu_set_index].status = SAMPLE_THREAD_RUNNING;
    while (1) {
        if (params->pmu_params->g_threads_meta_set[cpu_set_index].status == SAMPLE_THREAD_STOP) {
            break;
        }
        for (i = 0; i < EVENT_NUM; i++) {
...
static void pmu_threads_out(struct slide_params *params, int cpu_set_count)
{
    ...
    for (i = cpu_set_count - 1; i >= 0; i--) {
        if (params->pmu_params->g_threads_meta_set[i].tid == NULL) {
            continue;
        }
        params->pmu_params->g_threads_meta_set[i].status = SAMPLE_THREAD_STOP;
        pthread_join(*(params->pmu_params->g_threads_meta_set[i].tid), NULL);
        free(params->pmu_params->g_threads_meta_set[i].tid);
        params->pmu_params->g_threads_meta_set[i].tid = NULL;
    }
}
                                      ...
                                      struct perf_cpu_monitor **perf_cpu_monitors)
{
    ...
    for (i = 0; i < cpu_set_count; i++) {
        ...
        params->pmu_params->g_threads_meta_set[i].tid = (pthread_t *)calloc(1, sizeof(pthread_t));
        if (params->pmu_params->g_threads_meta_set[i].tid == NULL) {
            etmemd_log(ETMEMD_LOG_ERR, "malloc for g_threads_meta_set[%d].tid fail\n", i);
            goto start_thread_out;
        }

        ret = pthread_create(params->pmu_params->g_threads_meta_set[i].tid, NULL, pmu_sample_thread, &thread_args[i]);
        if (ret != 0) {
...
static int etmemd_start_sample_threads(struct task_pid *tk_pid)
{
    ...
    ret = update_page_refs_from_vma(tk_pid, &(params->pmu_params->g_page_refs), false);
    if (ret != 0) {
        goto init_vma_out;
    }
...
init_vma_out:
    free(params->pmu_params->g_threads_meta_set);
    params->pmu_params->g_threads_meta_set = NULL;
...
void merge_page_refs(struct task_pid *tpid, struct page_sort **page_sort, struct memory_grade **memory_grade)
{
    ...
    params->pmu_params->g_page_refs = page_refs_start;
}
...
void etmemd_stop_sample(struct task *tk)
{
    ...
    clear_vma_info_list(params);
    params->pmu_params->g_vma_updata_count = 0;
    params->pmu_params->g_page_refs = NULL;
}
...
struct page_refs *etmemd_do_sample(struct task_pid *tpid, const struct task *tk)
{
    ...
    params->pmu_params->g_vma_updata_count = (params->pmu_params->g_vma_updata_count + 1) % \
                                             (params->pmu_params->vma_updata_rate);
    if (params->pmu_params->g_vma_updata_count == 0) {
        ret = update_page_refs_from_vma(tpid, &(params->pmu_params->g_page_refs), true);
        if (ret != 0) {
            return NULL;
        }
    }
    ...
    sleep(loop * sleep_t);
    return params->pmu_params->g_page_refs;
}
#endif